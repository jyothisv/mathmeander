...
use mathmeander_core::model::{
    ...
    LinkType, MathExpression, ObjectStatus, ObjectType, ObjectVersion, Origin, ParseStatus,
    Provenance, SurfaceFormat, Tagging, Unit, UnitContent, UnitStatus,
};
...
/// slice-1c blocker `validate_inline` exists to make unreachable. Import must refuse it.
#[test]
fn import_rejects_width_bearing_inline_atom() {
    ...
    unit.content = UnitContent::Prose {
        ...
        inline: vec![Inline::Math {
            span: CharSpan { start: 3, end: 7 }, // NOT zero-width — banned
            expr: an_expr(0xc1),
        }],
    };
    let content = MathContent {
...
fn import_rejects_malformed_link() {
    ...
    assert!(import_mathpack(pack_value(graph)).is_err());
}
